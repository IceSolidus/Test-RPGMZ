 ...
 * rename the js file.
 * --------------------------Latest Version------------------------------------
 * Hi all, this latest version adds some listeners for print commands, which
 * can help [CGMZ] plugins behave correctly in case the game screen is printed.
 * This might include things like hiding cursor, etc.
 *
 * Version 1.41.0
 * - Added print listeners
 *
 * @command Initialize
 ...
 * rename the js file.
 * --------------------------Latest Version------------------------------------
 * Hi all, this latest version adds more options for parsing rumble json.
 *
 * Version 1.40.0
 * - Added more rumble parsing options
 *
 * @command Initialize
 * @text Inicializar 
...
var CGMZ = {};
CGMZ.Versions = {};
CGMZ.Versions["CGMZ Core"] = "1.40.0";
CGMZ.Core = {};
...
//-----------------------------------------------------------------------------
// Takes a JSON parameter with "X", "Y", "Width", and "Height", return rectangle object
//-----------------------------------------------------------------------------
CGMZ_Utils.parseRectangleJSON = function(json, callingPlugin = "[CGMZ] Core") {
	const defaultRect = {X:0,Y:0,Width:0,Height:0};
	const r = this.parseJSON(json, defaultRect, callingPlugin, "You had a Rectangle parameter with invalid JSON. It could not be read.");
	return new Rectangle(Number(r.X), Number(r.Y), Number(r.Width), Number(r.Height));
};
//-----------------------------------------------------------------------------
// Takes a JSON parameter with "Duration" "Weak Magnitude" "Strong Magnitude" "Start Delay" and "Force"
// In CGMZ plugins, Duration of 0 is ignored
//-----------------------------------------------------------------------------
CGMZ_Utils.parseRumbleJSON = function(rumbleJSON, callingPlugin = "[CGMZ] Core") {
	const defaultRumble = {"Start Delay": "0", "Duration": "0", "Weak Magnitude": "0.0", "Strong Magnitude": "0.0", "Force": "false"};
	...
	return {
		startDelay: Number(parseRumble["Start Delay"]) || 0,
		duration: Number(parseRumble.Duration) || 0,
		weakMagnitude: parseFloat(parseRumble["Weak Magnitude"]) || 0.0,
		strongMagnitude: parseFloat(parseRumble["Strong Magnitude"]) || 0.0,
		forceRumble: (parseRumble.Force === 'true')
	};
};
//-----------------------------------------------------------------------------
...
CGMZ_Utils.parseTTSJSON = function(TTSJSON, callingPlugin = "[CGMZ] Core") {
	...
	return {
		volume: parseFloat(parseTTS.Volume) || 0.0,
		pitch: parseFloat(parseTTS.Pitch) || 0.0,
		rate: parseFloat(parseTTS.Rate) || 0.0
	};
};
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Make label text taking into account automatic space
//-----------------------------------------------------------------------------
CGMZ_Utils.makeLabelText = function(label, color) {
	return `\\c[${color}]${label}${CGMZ.Core.SpaceCharacter}\\c[0]`;
};
//-----------------------------------------------------------------------------
// Make icon text, or return nothing if no icon
//-----------------------------------------------------------------------------
CGMZ_Utils.makeIconText = function(icon) {
	return (icon) ? `\\i[${icon}]` : '';
};
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Utility function to award the player currency, since it is a bit complicated when using [CGMZ] Currency System
// WIP as this will be cleaner if [CGMZ] Currency System aliases it in the future
//-----------------------------------------------------------------------------
CGMZ_Utils.awardCurrency = function(id, amount) {
	let awarded = false;
	if(id && Imported.CGMZ_CurrencySystem) {
...
//-----------------------------------------------------------------------------
// Route to proper lerp function based on mode
//-----------------------------------------------------------------------------
CGMZ_Utils.lerpRouter = function(start, end, percent, mode) {
	switch(mode) {
		case 'Linear': return this.lerp(start, end, percent);
		case 'Ease In': return this.lerp(start, end, this.lerpEaseIn(percent));
		case 'Ease Out': return this.lerp(start, end, this.lerpEaseOut(percent));
		case 'Ease In Out': return this.lerp(start, end, this.lerpEaseInOut(percent));
	}
	return this.lerp(start, end, percent);
};
//-----------------------------------------------------------------------------
...
CGMZ_Utils.initializeListeners = function() {
	...
	window.addEventListener("blur", this.onLoseFocus.bind(this));
	window.addEventListener("focus", this.onGainFocus.bind(this));
...
// Lose focus listener
//-----------------------------------------------------------------------------
CGMZ_Utils.onLoseFocus = function(event) {
	$cgmzTemp?.onLoseFocus(event);
};
//-----------------------------------------------------------------------------
// Gain focus listener
//-----------------------------------------------------------------------------
CGMZ_Utils.onGainFocus = function(event) {
	$cgmzTemp?.onGainFocus(event);
};
//-----------------------------------------------------------------------------
// Before Print listener
//-----------------------------------------------------------------------------
CGMZ_Utils.onBeforePrint = function(event) {
	$cgmzTemp?.onBeforePrint(event);
};
//-----------------------------------------------------------------------------
// After Print listener
//-----------------------------------------------------------------------------
CGMZ_Utils.onAfterPrint = function(event) {
	$cgmzTemp?.onAfterPrint(event);
};
//=============================================================================
// CGMZ_Temp
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
CGMZ_Temp.prototype.onLoseFocus = function(event) {
	//
};
//-----------------------------------------------------------------------------
// Called when focus is gained
//-----------------------------------------------------------------------------
CGMZ_Temp.prototype.onGainFocus = function(event) {
	//
};
//-----------------------------------------------------------------------------
// Called before printing the game screen
//-----------------------------------------------------------------------------
CGMZ_Temp.prototype.onBeforePrint = function(event) {
	//
};
//-----------------------------------------------------------------------------
// Called after print dialogue is closed (printed or canceled)
//-----------------------------------------------------------------------------
CGMZ_Temp.prototype.onAfterPrint = function(event) {
	//
};
//-----------------------------------------------------------------------------
...
Game_Map.prototype.CGMZ_getMapName = function() {
	return ($dataMap?.meta?.cgmzname) ? $dataMap.meta.cgmzname : "Unknown";
};
//-----------------------------------------------------------------------------
...
//=============================================================================
//-----------------------------------------------------------------------------
// Get the padding for a window. Aliased by other [CGMZ] plugins, but provided
// here as a base to ensure the function exists. id should be provided to
// find the proper id for [CGMZ] Window Settings
//-----------------------------------------------------------------------------
Scene_MenuBase.prototype.CGMZ_readWindowPadding = function(/*id*/) {
...
//-----------------------------------------------------------------------------
Scene_MenuBase.prototype.CGMZ_calcMixedHeight = function(selectableLines, staticLines, padding = -1) {
...
CGMZ_Sprite_Gauge.prototype.gaugeRate = function() {
	if(this._options.useCustomRate) return this.calculateCustomGaugeRate();
...
//-----------------------------------------------------------------------------
// Initialize the sprite
// Pass optional parameters in by ext. Supported Parameters:
// dir: Number. Direction the walk sprite will face, default is 2 (down)
// animate: Boolean. True / false if it should be animated, default is true.
//-----------------------------------------------------------------------------
CGMZ_Sprite_WindowAnimatedWalkSprite.prototype.initialize = function(name, index, ext = {}) {
	Sprite.prototype.initialize.call(this);
	this._characterName = name;
	this._characterIndex = index;
	this.initExtMembers(ext);
...
//-----------------------------------------------------------------------------
CGMZ_Sprite_WindowAnimatedWalkSprite.prototype.initExtMembers = function(ext) {
	this.anchor.x = 0;
	this.anchor.y = 0;
	this._direction = (ext.hasOwnProperty('dir')) ? ext.dir : CGMZ.CONSTANTS.DIRDOWN;
	this.animated = (ext.hasOwnProperty('animate')) ? ext.animate : true;
	this._pattern = 1;
	this._timer = 0;
};
//-----------------------------------------------------------------------------
...
// motion: Number. The battle motion to make, default is 13
// animate: Boolean. If the battler should be animated, default is true
//-----------------------------------------------------------------------------
...
// Init sprite variables
//-----------------------------------------------------------------------------
CGMZ_Sprite_WindowAnimatedBattleSprite.prototype.initExtMembers = function(ext) {
	this.anchor.x = 0;
	this.anchor.y = 0;
