 ...
 *
 * @command player_trail
 * @desc Ativar o efeit Ghost Trail no personagem.
 * @text Player
 *
 * @arg id
 ...
 *
 * @arg trails
 * @desc Define a quantidade de sombras.
 ...
 *
 * @command event_trail
 * @desc Ativar o efeit Ghost Trail no evento.
 * @text Event
 *
 * @arg id
 ...
 *
 * @arg trails
 * @desc Define a quantidade de sombras. (5 - 20)
 * @text Number of Trails
 * @default 5
 * @type number
 * @min 5
...
　　var Moghunter = Moghunter || {}; 
 ...
 　Moghunter.parameters = PluginManager.parameters('MOG_CharGhostTrail');  
  ...
  PluginManager.registerCommand('MOG_CharGhostTrail', "player_trail", data => {
    	  var char_id = Number(data.id);
		  var visible = String(data.visible) == "true" ? true : false;
		  var blend_mode =  Number(data.blend_mode);
		  var trails = (Math.min(Math.max(Number(data.trails),5),20));
		  var mode = String(data.mode);
	      $gameMap.setGhostTrailPlayer(char_id,visible,blend_mode,trails,mode);
   });	
   
  PluginManager.registerCommand('MOG_CharGhostTrail', "event_trail", data => {
    	  var char_id = Number(data.id);
		  var visible = String(data.visible) == "true" ? true : false;
		  var blend_mode = Number(data.blend_mode);
		  var trails = (Math.min(Math.max(Number(data.trails),5),20));
		  var mode = String(data.mode);
	      $gameMap.setGhostTrailEvent(char_id,visible,blend_mode,trails,mode);
   });	   	
...
CharGhostTrailSprites.prototype.createGhostTrail = function() {
	 ...
	 for (var i = 0; i < this.data().trails; i++) {	
		  ...
		  this._trailSprites[i]._blendMode = this.data().blendMode;
